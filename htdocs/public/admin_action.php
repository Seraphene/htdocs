...
if ($type === 'photo' && $id) {
    ...
    $stmt = $conn->prepare('SELECT StudentNo, StudentName FROM student WHERE StudentID = :id');
    $stmt->execute(['id' => $id]);
    $student = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($action === 'approve') {
        $stmt = $conn->prepare('UPDATE student SET PhotoConfirmed = 1 WHERE StudentID = :id');
        $stmt->execute(['id' => $id]);
        ...
        $stmt = $conn->prepare('UPDATE student SET ProfilePhoto = NULL, PhotoConfirmed = 0 WHERE StudentID = :id');
        $stmt->execute(['id' => $id]);
